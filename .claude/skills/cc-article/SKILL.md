---
name: cc-article
description: Claude Code / AI コーディング系の記事 (Zenn・Qiita・個人ブログ・英語) の URL を渡すと、その手法・Tips を claude-forge に採用できるかを診断する skill。各テクを ✅採用候補 / 🤔条件付き / ❌見送り で判定し、落とし所 (どの skill / 設定 / hook / agent になるか・既存との重複) と次アクションまで提示する (実装はしない)。判断材料として記事の要約も添える。ユーザーが「この記事 claude-forge に活かせる?」「<URL> をうちのリポジトリに採用できるか見て」「この Zenn/Qiita うちに取り込める?」「この手法 claude-forge に入れられる?」「この記事の Tips うちのリポジトリに反映できる?」などと、CC 関連記事の URL を渡して自リポジトリへの取り込み判断を求めたときに発動する。単に記事を要約・解説してほしいだけなら発動しない (素の Claude で足りる)。AWS / Claude 公式 docs を図解 HTML 教材にしたいだけなら [[doc-illustrate]] を使う。
allowed-tools: Read, Glob, Grep, WebFetch, Bash(python3:*), Bash(curl:*), Bash(ls:*)
---

# cc-article

Claude Code 関連の記事 (Zenn / Qiita / ブログ / 英語記事) の URL を渡されたら、**その手法を claude-forge に採用できるかを診断する** skill。**成果物は「採用診断」**（記事のテクを claude-forge の skill / 設定 / hook / agent のどこに落とせるか・既存と重複しないかの判定と次アクション）。記事の要約は、その診断を地に足の着いたものにするための**材料として添える**だけ。
...
> **要約だけが欲しいなら、この skill は不要**（素の Claude が直接やる）。この skill の存在価値は「自リポジトリへの取り込み判断」。だから発火トリガも「採用できる?/取り込める?/入れられる?」側に寄せている。確実に使いたい時は `/cc-article <URL>` と明示的に呼ぶ。
...
- 記事は **claude-forge と無関係なテクも混ざる**前提で、CC / skill / 設定 / 運用に効く部分を拾う（ここが診断の入力になる）。
...
### 3. claude-forge へのフィット診断（この skill の主役）
...
## 🔧 claude-forge へのフィット診断（← 本題）
| テク | 判定 | 落とし所 / 理由 |
|---|---|---|
| <テクA> | ✅採用候補 | 新規 skill 化。既存に無く方針とも整合 |
| <テクB> | 🤔条件付き | 既存 skill に統合が筋 / 要 MCP / 現行仕様か要検証 |
...
- **要約だけで終わらせない。** この skill の成果物は採用診断。要約は材料。診断と次アクションまで必ず出す。
...
- 取得・正確さの原則は [[doc-illustrate]] / [[aws-docs]] と同じ (生テキスト優先・主張は一次ソースで裏取り)。`extract.py` も [[doc-illustrate]] のものを流用。**役割は別**: doc-illustrate は公式 docs → 図解 HTML 教材を作る、cc-article は CC 記事 → 自リポジトリへの採用診断を返す。
