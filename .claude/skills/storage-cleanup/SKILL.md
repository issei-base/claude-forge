...
- **削除 (`rm`) は `allowed-tools` に入れていない = 毎回承認を挟む設計。** 完全無人で `rm` しない。
...
- `bash scripts/scan_storage.sh` を実行する（du スイープ + Tier 別の削除候補とサイズ + 起動中アプリ検出を一括で出力。**read-only**）。スクリプトが使えない環境なら `references/safe-targets.md` のパスを `du -sh` で個別に測る。
