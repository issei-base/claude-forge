...
| 設定/env / 外部リソース未準備 / flaky / シークレット不足 | 自動修正対象外。Step 8-5 で打ち切ってユーザに報告する |
...
`gh pr edit` を実行する直前に、ドラフトしたPR本文を自己点検する：

**推敲（簡潔さ）— 必ず1パス入れる:**
- [ ] 第三者が見て納得できる最低限の情報量に絞れているか
- [ ] 冗長な前置き・重複説明・蛇足を削ったか
- [ ] 修正分を追記しただけで終わらず、既存本文の冗長部も削ったか

**構造:**
- [ ] 上記「含めないもの」が混入していないか
- [ ] 空セクション（`<!-- -->` のみ等）を見出しごと削除済みか
- [ ] Changes セクションが diff の繰り返しになっていないか
- [ ] テンプレートの空欄を埋め忘れていないか
