...
- [ ] Phase 8.5: Codex 応答ループ（§4 参照・最大3サイクル）
...
PR URLから `owner/repo` を抽出し、ローカルにクローン済みかを確認する。

**Step 2-1: カレントディレクトリ確認**
...
カレントが対象リポジトリなら、そのまま使う。

**Step 2-2: ローカルリポジトリ探索**
...
- 見つかれば `cd` する
- 見つからなければ `gh repo clone <owner>/<repo> ~/projects/<リポジトリ名>` でクローンする
...
check が最後まで 1 つも現れなければこの CI Phase をスキップして Phase 8.5（Codex 応答ループ）へ。現れたら Step 8-2 の完了待ちへ。
...
- exit 0 → 全 PASS。Phase 8.5（Codex 応答ループ）へ。
...
## Phase 8.5: Codex 応答ループ

push・CI ループ通過後、Codex GitHub review の指摘へ自律対応する（worktree ディレクトリ内で実行）。
**手順・分類・収束/停止・禁止事項は [`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §4 に従う**（create-pr と同一定義）。要点（fallback）:
- Codex のレビューコメントを取得 → 各指摘を「自動修正可 / 要人間判断」に分類 → 自動修正可のみ working tree を直し commit + push → `@codex review` を再依頼。
- 停止: 新規 critical/blocking が 0 / **最大 3 サイクル**（CI ループと同数）/ 残りが要人間判断のみ。残った要人間判断は要約して Phase 10 でユーザーに引き継ぐ。
- **禁止**: 指摘を黙らせるためのテスト書き換え・握りつぶし。**ループは PR の ready/draft 状態を変えない・merge もしない**（§4 の「ready なら ready・draft なら draft」に従う）。force push しない。同意できない指摘は dispute としてコメントし、直さない。

> **役割分離:** §4（Phase 8.5）は **push 後の外部レビュー（Codex）収束**、Phase 6 のローカル `doc-impl-reviewer` ループは **push 前の自己検証**。両者は対象（外部 vs 自己）とタイミング（push 後 vs 前）が異なり、二重化しない。

Codex review が無効 / コメントが付かない repo ならこの Phase をスキップして Phase 9 へ。
...
### Codex 応答結果（Phase 8.5を実行した場合のみ）
- 対応サイクル: N回 / 停止理由: [新規 blocking なし / 上限到達 / 要人間判断のみ / Codexなし]
- 要人間判断で引き継ぐ指摘: <あれば要約。なければ「なし」>
```
...
| CI 未設定（`no checks reported`） | Phase 8 をスキップして Phase 8.5（Codex 応答ループ）へ |
...
3. **force pushしない** — ユーザの明示指示があるときだけ
4. **`gh pr merge` しない** — merge は人間が手動
5. **PR本文は全面更新かつ簡潔** — 追記でだらだら伸ばさない。create-prの簡潔さの方針を遵守
6. **Issueコメントは既存編集を優先** — 新規追加投稿は編集対象が無いときだけ
7. **ユーザ介入なしで完走** — エラー時のみ停止して報告
8. **ローカル変更の自動破棄禁止** — worktreeに未コミット変更があった場合は必ずユーザに判断を仰ぐ
9. **テストを passing に書き換えない** — CI 自動修正でテスト失敗を直すときは仕様/実装の正しさを判断する
