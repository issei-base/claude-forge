...
抽出と状態コミットを「単一実行＋成功後コミット」の2フェーズで行う:
  ...
  フェーズ1（抽出）: メッセージを stdout に出し、進めるべき状態を pending ファイルに書く。
    ...
    --all-projects を付けると ~/.claude/projects/ 配下の全プロジェクトを横断する
    （session_id はファイル名(UUID)で全プロジェクト一意なので、しおり state はそのまま使える）。
...
# 認証情報らしき文字列を含むメッセージは「興味シグナル」ではなく、かつ機微情報。
# 抽出段階で丸ごと捨て、LLM 分類にも interest-log.jsonl にも一切渡さない（高確度パターンのみ）。
SECRET_PATTERNS = (
    r"sk-ant-[A-Za-z0-9_\-]{20,}",          # Anthropic API / OAuth token
    r"A(?:KIA|SIA)[0-9A-Z]{16}",            # AWS access key id（恒久 / 一時）
    r"gh[pousr]_[A-Za-z0-9]{20,}",          # GitHub token
    r"github_pat_[A-Za-z0-9_]{20,}",        # GitHub fine-grained PAT
    r"sk-[A-Za-z0-9]{32,}",                 # OpenAI 形式の key
    ...
    with open(filepath, encoding="utf-8") as f:
        for i, line in enumerate(f):
            ...
            if looks_like_secret(content):
                # 認証情報を含むメッセージは抽出しない（LLM にもログにも渡さない）。
                continue
...
def main():
    ...
    parser.add_argument("--all-projects", action="store_true",
                        help="~/.claude/projects/ 配下の全プロジェクトのログを横断して解析する（既定はカレントプロジェクトのみ）")
    ...
    # 抽出フェーズ: 解析対象の .jsonl を集める
    #   --logs-dir 指定 … そのディレクトリのみ
    #   --all-projects … ~/.claude/projects/ 配下の全プロジェクトを横断
    #   既定          … カレントプロジェクトのログのみ
    if args.logs_dir:
        search_dirs = [Path(args.logs_dir)]
    elif args.all_projects:
        root = Path.home() / ".claude" / "projects"
        search_dirs = sorted(p for p in root.glob("*") if p.is_dir()) if root.exists() else []
    else:
        search_dirs = [default_logs_dir()]

    # 単一ディレクトリ指定（--logs-dir / 既定）で存在しなければ従来どおり明示エラー。
    # --all-projects は対象が無くても 0 件で進める（自律実行で止めない）。
    if not args.all_projects and (not search_dirs or not search_dirs[0].exists()):
        target = search_dirs[0] if search_dirs else "(none)"
        print(f"ERROR: logs dir not found: {target}\n"
    ...
    # session_id = ファイル名(UUID) は全プロジェクトで一意なので、複数ディレクトリを
    # 束ねても per-session のしおり (mtime/lines_read) はそのまま機能する。
    jsonl_files = sorted(
        (f for d in search_dirs if d.exists() for f in d.glob("*.jsonl")),
