...
- env `GIT_PUSH_GUARD_DISABLE=1` で repo 単位のオプトアウトができる
  (main 直 commit を許容する solo private repo 向け。claude-forge-personal が
  `.claude/settings.json` の `env` で宣言する。global hook として全 repo に
  効かせる運用と、repo ごとの方針差を両立させるための逃げ道)。
...
def main() -> int:
    if os.environ.get("GIT_PUSH_GUARD_DISABLE") == "1":
        return 0  # repo 単位のオプトアウト (docstring「スコープは意図的に最小」参照)
