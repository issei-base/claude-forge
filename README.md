...
| Skill | 発動するフレーズ | 中身 |
|---|---|---|
| `ship` | 「ship して」「PR出して」「PR作って」 | 変更を branch→commit→push→PR まで対話的に。**default branch への直接 push を hard guard で禁止** |
| `codex-review` | 「codex にレビュー」「セカンドオピニオン」「adversarial レビュー」 | 現在の差分を Codex CLI に渡して逐語提示。設計・セキュリティを敵対的に突く **adversarial モード**も持つ |
...
| `cc-article` | 「この記事 claude-forge に活かせる?」「この Zenn/Qiita うちに取り込める?」 | CC 関連記事の URL を読み、**claude-forge に採用できるか診断**（✅/🤔/❌ + 落とし所 + 次アクション。実装はしない） |
