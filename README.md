...
├── .github/workflows/          # claude-forge 自身の PR 自動レビュー / セキュリティ / skill-lint (CI lint)
├── install.sh                  # 全 skill + agent を ~/.claude に symlink (global 運用)。--dry-run 可
├── data/                       # skill の実行時生成物 (x-buzz ログ等)。gitignore 済み
...
| `x-buzz` | 「X投稿作って」「今日のポスト考えて」「Claude ネタでツイート」「ツイート量産」 | Claude / AI コーディングの X 投稿をインプ目的で量産。C(Tips)+D(逆張り)+海外バズの型を**本物ネタ**(changelog / 実物 skill / 実務)で埋める。reach を削る要因 (本文 URL・ハッシュタグ過多・丸コピ・重複) を `scripts/validate_post.py` で機械検証してから [Typefully MCP](https://support.typefully.com/en/articles/13128440-typefully-mcp-server) に下書き/予約 push (未導入時はローカルキューに書き出し)。**本数より質** (高頻度連投は throttle されるので欲張らない)。投稿ログ (`data/x_buzz_posted.jsonl` / `data/x_buzz_queue/`) は gitignore 済み。定期化は `schedule` skill と併用 |

### Issue → 計画 → 実装 → PR ワークフロー (SRE チーム由来・個人開発向けに改変)

本業の SRE チームで使っていた一連のスキルを個人開発向けに移植・ブラッシュアップしたもの。Issue 管理から実装・PR まで一気通貫で繋がる。社内固有部分 (`/workspace` パス・特定の社内リポジトリ・専用 Team ツール) は、カレント/`~/projects/` 配下のローカルリポジトリ・`gh`・`Agent` tool ベースに置き換え済み。コードレビューは `doc-impl-reviewer` agent、計画/ドキュメントの多観点レビューは `doc-review` skill (中核は `doc-reviewer` agent に委譲) が担当する。

| Skill | 発動するフレーズ | 中身 |
|---|---|---|
| `create-sre-issue` | 「Issue作って」「タスク登録して」「この Issue リライトして」 | カレント (or 指定) リポジトリの GitHub Issue を新規作成 / リライト。Issue 本文は**概要だけ**に絞る (実装詳細は `plan-sre-issue` 側へ)。投稿前に冗長セクション混入をセルフチェック。名前に "sre" が付くが対象は任意リポジトリ |
| `plan-sre-issue` | 「実装計画を立てて」「設計して」(Issue URL とともに) | Issue を分析→インタビュー→コードベース調査 (`Agent`/Explore)→第三者が実装できる計画を **Issue コメント**として投稿。投稿前に `doc-review` でレビュー。雛形は同梱 `references/plan-template.md` |
| `implement-sre-issue` | 「実装して」「これやって」(Issue URL / ドキュメントパスとともに) | 要件取得→対象リポ特定→**worktree で**実装→`doc-impl-reviewer` レビュー (最大5回)→`create-pr` で PR 作成→Issue コメントまで自動完走 |
| `fix-pr` | 「PR修正して」「レビュー指摘対応して」(PR URL とともに) | 既存 PR を worktree で修正→push→**CI 自動修正ループ**→PR 本文・Issue コメント更新まで自動完走 |
| `create-pr` | 「/create-pr」/ 他スキルからの委譲 | コミット→push→PR 作成→**CI 失敗の自動修正ループ**まで止まらず実行する PR エンジン。`claude-review` ラベルがある repo では自動付与。対話的に出したいなら `ship`、これは `implement-sre-issue`/`fix-pr` 用のエンジン |
| `doc-review` | 「/doc-review」/ `plan-sre-issue` からの委譲 | **多観点レビュー*ワークフロー***。中核を `doc-reviewer` agent に委譲しつつ security/ops/cost を `Agent` tool で並列追加起動し、リーダーが集約・修正・再レビュー (最大3ラウンド)。単発の軽いドキュメントレビューは `doc-reviewer` agent を直接使う |
...
> 本業の SRE チームで使っていた 2 本を、個人開発向けに standalone 化してブラッシュアップしたもの。親 skill 依存・チーム運用前提を外し、[公式 subagent docs](https://code.claude.com/docs/en/sub-agents) のベストプラクティス (focused / proactive な description・最小ツール・`git diff` 起点) に揃え、確信度フィルタと一次ソース検証を追加。
...
2. `Claude PR Review` / `Claude Security Review` workflow が起動 → **inline コメント + 指摘サマリーコメント**を post する。`gh pr review` での approve/request-changes は付けない (action v1 の sandbox で安定しないため割り切り済み・下記 Branch Protection 節参照)
