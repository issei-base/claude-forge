...
**読み方** — 形と色で役割を表す: 入口/振り分け＝丸み（黄）、PR 生成エンジン＝二重枠（青）、通常の skill＝角丸（淡灰）、AWS・教材＝緑、agent（委譲先）＝六角（紫）。**実線＝流れ・受け渡し、点線＝agent への委譲。** 細部は下の各表を参照。

```mermaid
flowchart TB
    classDef router fill:#fef3c7,stroke:#d97706,stroke-width:2px,color:#78350f;
    classDef engine fill:#dbeafe,stroke:#2563eb,stroke-width:2px,color:#1e3a8a;
    classDef skill  fill:#f8fafc,stroke:#94a3b8,color:#0f172a;
