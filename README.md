...
| `create-pr` | 「/create-pr」/ 他スキルからの委譲 | コミット→push→PR 作成→**CI 失敗の自動修正ループ**まで止まらず実行する PR エンジン。`claude-review` ラベルがある repo では自動付与。対話的に出したいなら `ship`、これは `implement-issue`/`fix-pr` 用のエンジン |
