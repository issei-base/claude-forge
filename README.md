...
│   │   ├── skills/             #   create-issue, weekly-plan, plan-issue, implement-issue,
│   │   │                       #   orchestrate, create-pr, ship, fix-pr, _shared
...
│   │   ├── skills/             #   banso, grill-me, spike, tdd, doc-review, aws-docs
...
| merge 判断 | 人間 + 常設ループ | 原則は人間が GitHub UI で内容を見て手動 merge。**allowlist に登録した repo だけ**、常設ループ（claude-forge-personal の `pr_watch.py`）が安全ガード（ラベル・パス・CI 緑）を通った PR を自動 merge する |

2026-07-11 に「Codex はレビュー専任・PR を作らない」を撤回し、Codex も branch / commit / push / PR 作成をしてよい（merge の扱いは上表のとおり）。旧 review-only stub（`.agents/skills/ship`）は削除済み。
...
| `grill-me` | 「グリルして」「質問攻めにして」「尋問して」「この計画を質問で詰めて」 | 実装前の計画・企画・PRD を**一問一答の尋問**で固める（Matt Pocock の grilling を土台に改良）。手戻りコストが高い分岐から質問・推奨案付き・事実は調べて決定だけ問う・5 問ごとに決定マップ・矛盾検知。固定した用語は（初回 1 回の確認のうえ）対象 repo の `docs/GLOSSARY.md` へ都度書き戻す。締めは決定サマリー → ADR / `plan-issue` / `banso` へ橋渡し（**実装しない**） |
| `tdd` | （model-invoked）issue を介さない実装・バグ修正の依頼で自動発火 | アドホックなコード作業の **TDD 規律**。振る舞いを自然言語ケースで先に列挙 → テストを書いて**先に落とす（red）**→ 最小実装で通す（green）→ 通ったまま整える。バグ修正は再現テストが red。例外（回帰固定・基盤なし・docs のみ）は申告必須。issue 起点は `implement-issue`/`orchestrate` が自前の手順を持つ |
...
| Skill | 発動するフレーズ | 中身 |
|---|---|---|
| `create-issue` | 「Issue作って」「タスク登録して」「この Issue リライトして」 | GitHub Issue を新規作成 / リライト。本文は**概要だけ**に絞る（実装詳細は `plan-issue` 側へ）。Status は `Backlog` 止まり（今週やるかは `weekly-plan` が決める） |
| `weekly-plan` | 「今週の計画立てて」「週初めのトリアージして」「Todo に積んで」 | 週初めの**器決め**。前週の積み残しを棚卸し → Milestone・優先度で候補を並べ → 今週の器に入る分だけを提案 → 承認後に Status を **`Todo`** へ一括更新（夜間に回せるものは `nightly` ラベルも）。着手の入口である Todo を満たすのが役割で、**実装はしない** |
| `plan-issue` | 「実装計画を立てて」「設計して」(Issue URL とともに) | Issue を調査し、第三者が実装できる計画（**機械判定できる完了条件**を含む）を **Issue コメント**として投稿（投稿前に `doc-review`） |
| `implement-issue` | 「実装して」「これやって」(Issue URL / ドキュメントパスとともに) | 要件取得→**worktree で**計画提示（自然言語テストケース一覧つき）→**テスト先行（red→green）**で実装→`doc-impl-reviewer` レビュー→`create-pr` で PR→Issue コメントまで自動完走 |
| `orchestrate` | 「#12 #13 まとめて着手して」「並列でやって」(複数 issue とともに) | 複数 issue を**2 段 fan-out** — 計画→**✋テストレビューゲート（唯一の停止点）**→並列実装（Opus×worktree・上限 3 レーン・repo 混在可）→**CI 収束監視**（赤は `fix-pr` を 1 回だけ）→運行ボード形式で報告。merge はこの skill では行わない。単発 issue は `implement-issue` へ |
...
| `skill-reviewer` | **SKILL.md** を claude-forge の craft 規約 (CLAUDE.md「約束事」+ `_template`「書き方の作法」) でレビュー。決定的 lint (E1–E8/W1–W5) が見られない**発火設計 / 断定の書き方 / 順序つき手順 / 重複・粒度 / 方針整合**を確信度 80 以上で指摘し JSON で返す。lint の指摘は再掲せず、**SKILL.md は変更しない** | 「この skill レビューして」「SKILL.md の作法チェック」「発火が誤らないか」、新規/改修 skill を ship する前 |
...
**skill は merge しない。** Codex も Claude の PR skill（`ship` / `create-pr`）も merge を絶対に行わない。判断するのは人間か、**allowlist に登録した repo では常設ループの安全ガード**（`_shared/pr-conventions.md` §0）。
...
3. **merge**: 人間が内容を見て手動で行う。**allowlist に登録した repo だけ**は常設ループ（`pr_watch.py`）が CI 緑・安全ガード通過を条件に無人で merge する
