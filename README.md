...
├── .github/workflows/          # skill-lint (CI lint) のみ。PR レビューは push 前のローカル codex-secondopinion
...
claude-forge では **PR 作成は Claude Code、PR レビューは Codex** に分ける。レビューは GitHub 側の automatic review ではなく、**push 前のローカル Codex CLI**（`codex-secondopinion`）で行う（2026-07 方針転換）。

| 役割 | 担当 | 発火/操作 |
|---|---|---|
| PR 作成 | Claude Code | Claude の `ship` skill / `create-pr` skill |
| PR レビュー | Codex（ローカル CLI） | `ship` の push 前確認 → `codex-secondopinion`。GitHub 側 automatic review は既定オフ |
...
| `.github/workflows/` | `skill-lint.yml` (CI lint) のみ。PR レビューは push 前のローカル `codex-secondopinion` |
...
| `install-pr-reviews` | 「この repo にも PR レビュー入れて」「Codex review 入れて」 | cwd の GitHub repo で Codex GitHub code review を**オプトイン**有効化する設定確認と案内（既定はローカル `codex-secondopinion`。GitHub 側 review が要る repo だけ） |
...
標準は **push 前のローカル Codex CLI レビュー**（`codex-secondopinion`）。`ship` が push 直前に「Codex レビューをかける?」を 1 回確認し、YES なら `codex review` をローカル差分にかけてから PR を出す（PR の初版を Codex-clean にする前倒し）。

**2026-07 方針転換**: 以前は Codex GitHub automatic review + PR 作成後の応答ループ（最大 3 サイクル）を標準にしていたが、トークン消費とレビュー到着待ち（30 秒 polling × 最大 ~5 分 × サイクル数）が大きく廃止した。**PR 作成後に `@codex review` は投げない**。

| 方式 | 位置づけ |
|---|---|
| `codex-secondopinion`（ローカル） | 標準。ship の push 前確認で YES と答えたとき / 「codex にレビュー」と明示したとき |
| Codex GitHub automatic reviews | オプトイン。GitHub 側の自動レビューも欲しい repo だけ `install-pr-reviews` で有効化（応答ループは回さない・指摘対応は人間が読んで `fix-pr` に依頼） |
...
1. Claude Code の `ship` skill で push 直前に Codex ローカルレビューの要否を確認（要るなら `codex-secondopinion` を実行して差分を直してから push）
...
ローカルの `codex-secondopinion` はどの repo でもそのまま使える（cwd の git 差分が対象・repo 側の準備は不要）。GitHub 側の automatic review も欲しい repo だけ、その cwd で **「この repo にも PR レビュー入れて」** と言うと `install-pr-reviews` skill が発火し、既定（ローカル事前レビュー）の確認の上で Codex GitHub code review の有効化手順と `AGENTS.md` Review Guidelines を案内する。明示的に呼びたい時は `/install-pr-reviews` でも OK。
