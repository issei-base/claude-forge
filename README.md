...
> **スコープ: この repo は開発フロー系 skill のみ。** 個人用途の skill (受講生レッスン・記事消化・Mac 環境メンテ等) は別の private repo に分離してある (2026-07-06)。両 repo とも同じ `install.sh` 方式で `~/.claude/skills/` へ symlink するので、実働はその union になる。
...
- **受講生の個人情報・非公開 URL** — 永続 memory や repo に残さない。commit 前の混入監査は `leak-auditor` agent が担う
- **個人用途の skill とその生成物** (受講生レッスン・記事消化・Mac メンテ等) — 別の private repo (claude-forge-personal) で管理し、この repo には置かない (2026-07-06 分離)
