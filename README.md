...
| `orchestrate` | 「#12 #13 まとめて着手して」「並列でやって」(複数 issue とともに) | 複数 issue を**2 段 fan-out** — 計画→**✋テストレビューゲート（唯一の停止点）**→並列実装（Opus×worktree・上限 3 レーン・repo 混在可）→**CI 収束監視**（赤は `fix-pr` を 1 回だけ）→運行ボード形式で報告。merge は人間。単発 issue は `implement-issue` へ |
