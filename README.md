...
| `install-pr-reviews` | 「この repo にも PR レビュー入れて」「Codex review 入れて」 | 現在 cwd の GitHub repo で Codex GitHub code review を有効化するための設定確認と案内。`AGENTS.md` の Review Guidelines を整え、automatic reviews / `@codex review` 運用へ寄せる |
...
| `storage-cleanup` | 「ストレージを最適化して」「ディスクの空きを増やして」「不要なキャッシュを調査して削除して」「定期的にストレージ掃除して」 | Mac のローカルストレージを **調査 → 安全確認 → 削除** の 3 フェーズで段階的に最適化。読み取り専用の `scripts/scan_storage.sh` が du スイープ + Tier 別候補 + 起動中アプリ検出を出し、削除分類の根拠は `references/safe-targets.md` に集約。**起動中アプリの Caches は skip / 追記中ログは truncate / 実データ (Tier 3) は触らない**安全設計で、削除前後の `df` で回収容量を報告。削除 (`rm`) は `allowed-tools` 外＝毎回承認を挟む（無人 cron で `rm` しない）。定期実行は Phase 1–2（調査+レポート）まで自動化し削除の GO は人間。クラウド/AWS ストレージは `aws-advisor` |
| `spike` | 「WebSocket と SSE どっち採用すべき?」「この方式実現可能か調べて」「採用判断のためにスパイクして GO/NO-GO 出して」「PoC 的に小さく試して判断材料を出して」 | 実装に入る前に「やるべきか / どの方式か」を決める**タイムボックス調査**。問いを検証可能な命題に固め → コード/Web/小さな使い捨て検証で証拠を集め → 有力案を `codex-review` に敵対的に当て → `GO` / `NO-GO` / `INCONCLUSIVE` を根拠・トレードオフ・残リスク付きで**チャットに**返す（希望時のみ `~/Downloads` にファイル化）。**実装計画ではなく意思決定ドキュメント**が成果物 ── 作ると決めた後の計画は `plan-issue`。AWS の論点は `aws-docs`/`aws-advisor` で一次ソース化 |
| `gemini-multimodal` | 「この mp4 を要約して」「録音を文字起こしして」「動画の要点をタイムスタンプ付きで」「PDF から仕様抽出して」「このスクショ解析して」 | 音声・動画・PDF・画像の解析を **Gemini CLI** (`gemini -p "… @<path>"`) に委譲。**主用途は Claude が処理できない音声・動画**の文字起こし/要約（PDF・画像は単発なら Claude の `Read` を優先）。Preflight で `which gemini` を確認し、**Login with Google・`GEMINI_API_KEY` 不使用**（API 従量課金回避・サブスク/無料枠内なら無課金）を案内。出力は逐語で提示。テキスト/コードは Claude、図解 HTML は `doc-illustrate`。要 `npm i -g @google/gemini-cli` |
