...
| `playbook-sync` | 「これをチームの AI 活用プレイブックに入れて」「この内容を ai-team-playbook に反映して」 | テキスト/リンクを生テキストで取得し、**チームの規範 repo `issei-base/ai-team-playbook`** の該当テーマ md に**あるべき論として蒸留・統合**して main に直接 push（`extract.py` 流用・社内固有情報は落とす。CC 環境への取り込み診断は `cc-article`・チャット解説は `explain-article`） |
