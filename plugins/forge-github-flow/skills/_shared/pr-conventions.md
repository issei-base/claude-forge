...
- 関連 Issue / PR / ドキュメントへのリンク。**その PR で完了する Issue は `Closes #<番号>` で書く**（merge 時に GitHub が Issue を自動 close する。複数あれば並記）。完了させない関連参照は `Related: #<番号>`。裸の `#<番号>` 参照だけでは close されないので、完了 PR で Closes を省略しない
