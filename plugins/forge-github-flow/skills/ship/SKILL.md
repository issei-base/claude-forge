...
allowed-tools: Read, Edit, Write, Glob, Grep, Skill, Agent, Bash(grep:*), Bash(git status:*), Bash(git remote:*), Bash(git ls-remote:*), Bash(git log:*), Bash(git branch:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(git checkout:*), Bash(git add:*), Bash(git reset:*), Bash(git restore:*), Bash(git commit:*), Bash(git push:*), Bash(gh auth status:*), Bash(gh repo view:*), Bash(gh repo create:*), Bash(gh pr view:*), Bash(gh pr create:*), Bash(gh pr checks:*), Bash(gh run list:*), Bash(gh run view:*), Bash(sleep:*), Bash(prettier:*), Bash(npx prettier:*), Bash(eslint:*), Bash(npx eslint:*), Bash(ruff:*), Bash(golangci-lint:*), Bash(terraform fmt:*), Bash(terraform validate:*)
---
...
- 完了待ち（§5 要点）: **`timeout` / `gtimeout` は使わない** — macOS に標準では入っておらず、`timeout 900 …` は CI を待たずに即座に抜ける。`bucket` が `pending` の check が消えるまで 30 秒間隔・最大 30 回（15 分）待つ。FAIL なら `gh run list` → `gh run view <run-id> --log-failed` で失敗ログのファイル・行を取得し、§3 に従って根本修正（推測修正しない）→再 push。最大 3 サイクル。
