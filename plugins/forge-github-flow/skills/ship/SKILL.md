...
allowed-tools: Read, Edit, Write, Glob, Grep, Skill, Agent, Bash(grep:*), Bash(git status:*), Bash(git remote:*), Bash(git ls-remote:*), Bash(git log:*), Bash(git branch:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(git checkout:*), Bash(git add:*), Bash(git reset:*), Bash(git restore:*), Bash(git commit:*), Bash(git push:*), Bash(gh auth status:*), Bash(gh repo view:*), Bash(gh repo create:*), Bash(gh pr view:*), Bash(gh pr create:*), Bash(gh pr checks:*), Bash(gh run list:*), Bash(gh run view:*), Bash(sleep:*), Bash(jq:*), Bash(tr:*), Bash(prettier:*), Bash(npx prettier:*), Bash(eslint:*), Bash(npx eslint:*), Bash(ruff:*), Bash(golangci-lint:*), Bash(terraform fmt:*), Bash(terraform validate:*)
---
...
**発火条件を厳密に判定する。** branch の diff が SKILL.md を**追加/変更**していて、かつ `skill-reviewer` agent が使える時**のみ**このゲートに入る。それ以外（SKILL.md を触っていない・`_template` だけ・agent が無い repo）は**丸ごとスキップ**する。**実体は `plugins/<plugin>/skills/` にあり `.claude/skills/<name>` は symlink なので、`.claude/` 側だけを見るとヒットしない**（本文を直しても symlink 自体は変わらない）。両方の置き場所を見る:

```sh
git diff <base>...HEAD --name-only | grep -E '(plugins/[^/]+/skills|\.claude/skills)/[^_/][^/]*/SKILL\.md$'
```
...
> lint の指摘（E1–E8/W1–W5）はこのゲートの対象外。それは Stop hook / CI が別途担保する。ここは「lint が見られない判断レイヤ」だけを見る。
...
- 完了待ち（§5 要点）: **`timeout` / `gtimeout` は使わない** — macOS に標準では入っておらず、`timeout 900 …` は CI を待たずに即座に抜ける。§5 の完了待ちブロック（1 回 ~3.5 分）を、`DONE` / `GH_ERROR` が出るまで最大 4 回呼び直す。**`gh pr checks` は pending 中も非ゼロ（exit 8）で終わる**ので、`||` で失敗扱いにしない（CI が走り始めた瞬間に打ち切られる）。FAIL なら `gh run list` → `gh run view <run-id> --log-failed` で失敗ログのファイル・行を取得し、§3 に従って根本修正（推測修正しない）→再 push。最大 3 サイクル。
- 打ち切り（§5-4 要点）: **前サイクルと失敗 check 名の集合が完全一致したら、サイクルが残っていても打ち切る**。同じ失敗を 3 周しても直らないので、残りを回さず §6 の報告でユーザーに引き継ぐ。
...
- Review コメントは **参考情報**。claude-forge では Branch Protection 無しの運用なので、merge の判断はリポジトリ側の条件に委ねる (Branch Protection を有効化している repo では、その条件に従う)。
- **この skill は merge も ready 昇格もしない**（収束ループでの修正 push は §5.5 の範囲内・force push はしない）。merge は人間か、**allowlist repo では常設ループの安全ガードが判断する**（[`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §0）。**「必ずユーザーが手動で merge する」とは報告しない** — allowlist repo では CI 緑・安全ガード通過を条件に無人で merge されうるため、放置すれば見ないまま入る。ここで終了。
