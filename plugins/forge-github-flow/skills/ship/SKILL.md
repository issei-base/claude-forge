...
push の直前に「**Codex のローカルレビュー（[`codex-secondopinion`](../codex-secondopinion/SKILL.md)）をかけてから push する?**」を**ユーザーに 1 回だけ**確認する（`_shared/pr-conventions.md` §4 が唯一の定義）。

- **YES** → `codex-secondopinion` skill を**別ステップで**実行する（出力の verbatim 表示 → triage までを読んでから続行。自動連結しない）。accept 相当の指摘を直した場合は §3.5 の `git reset --soft HEAD~1` 手順で commit を作り直してから §4 へ。狙いは「PR の初版を Codex-clean にする」前倒し。
- **NO** → そのまま §4 へ。
- **聞かなくてよいケース**: ① ユーザーが事前に「codex は不要」「そのまま出して」と明示済み → NO 扱いで §4 へ ② ユーザーが事前に「Codex にも先に見せて」「ship 前に codex レビュー」等と明示済み → YES 扱いで即実行 ③ この ship の中で既に codex-secondopinion を実行済み → 二重で聞かない・§4 へ。
...
- **§3.8 の Codex 事前レビュー**を実施したか（実施 / ユーザーが不要と回答 / 明示済みでスキップ）と、実施した場合の要点（accept で直した件数・dispute / defer で残した指摘）を明記する。
