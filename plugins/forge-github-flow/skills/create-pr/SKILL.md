...
PR は **常に ready（非 draft）** で作成する。merge は人間か、**allowlist repo では**常設ループの安全ガードが判断する（[`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §0）。

**この skill は draft PR を作らない**（2026-07-19 変更）。以前はレビュー予算切れの委譲を受けて `--draft` を付けていたが、draft だと CI 側のレビュー（`claude-review` 等は `draft == false` を条件にしていることが多い）が走らず、一番怪しい PR が無検査のまま人手に渡る。レビュー未解消の signal は **この skill が Step 5-6 で** `do-not-merge` ラベル＋コメントとして残す。
...
### 5-6: レビュー未解消の signal（呼び出し元が `レビュー未解消:` を渡したときだけ）

**`gh pr create` の直後・Step 6 の CI ループに入る前に実行する。順序が要点。** CI ループは最大 3 サイクル回りうるが、その待機中に CI が緑になると allowlist repo では常設ループ（[`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §0）が**先に merge しうる**。ラベルを CI ループの後に回すと、レビュー未解消の PR が無検査で main に入る。

呼び出し元（implement-issue Phase 8 等）が args に `レビュー未解消: <未解消指摘の要点>` を含めてきたときだけ実行する。**渡されていなければ何もしない。**

```bash
# 1. ラベル（常設ループの自動 merge を止める。コメントだけでは止まらない）
...
**完了待ち・判定・無進捗検知のコードは [`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §5 が唯一の定義**（ここに再掲せず、§5 のブロックをそのまま使う）。分岐の要点（fallback）:

- **`timeout` / `gtimeout` は使わない** — macOS に標準では入っておらず、`timeout 900 …` は CI を待たずに即座に抜ける。待ちは `for` + `sleep` で数え、1 ブロック ~3.5 分を最大 4 回呼び直す。
- `now` が `GH_ERROR`（`gh` が非ゼロ終了）→ **空文字列を「全 PASS」に倒さない。** CI 状態を一度も取得できていないので、Step 6-5 で「CI 状態不明」として打ち切る。
- `now` が空 → 全 PASS。Step 7（完了報告）へ。
- `now` が非空 → FAIL あり。Step 6-3 へ。**`prev` と完全一致するなら無進捗**なので Step 6-5 で打ち切る。
- 4 回呼び直しても pending が残る → タイムアウト。Step 7 の完了報告で「CI 未完了」を明記してユーザに引き継ぐ（PR は維持）。
...
- **無進捗**（`prev` と `now` の失敗 check 名が完全一致）→ サイクルが残っていても打ち切り、「無進捗のため打ち切り」を明記して Step 7 へ（[`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §5-4）。
- **CI 状態不明**（`now` が `GH_ERROR`）→ 全 PASS に倒さず打ち切り、`gh` が失敗した旨を明記して Step 7 へ。
...
**この skill は merge しない** — merge は人間か、allowlist repo では常設ループの安全ガードが判断する（[`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §0）。**「必ず人間が見てから merge される」とは報告しない** — allowlist repo では CI 緑・安全ガード通過を条件に無人で merge されうるため。
...
4. **`gh pr merge` しない** - merge は人間か、allowlist repo では常設ループの安全ガードが判断する（`_shared/pr-conventions.md` §0）
