...
```
- [ ] Phase 1: 入力判定・要件取得・着手宣言（Issue を In Progress に）
- [ ] Phase 2: 対象リポジトリ特定
- [ ] Phase 3: コードベース調査
...
- [ ] Phase 9: Waiting 遷移（理由コメントとセット）・完了報告
```
...
```bash
# issue が載っている Project・item・Status フィールドを 1 クエリで取得
...
**判断事項を要件サマリーの直後・冒頭近くに置く。** データモデル・型インターフェース・ユーザーに見える挙動など「ユーザが修正指示を出しそうな判断」ほど先に出し、機械的なリファクタリングや自明な作業は末尾に回す（下記 2. の項目内の並び順もこの基準に従う）。このスキルは停止しないため、割り込みが間に合うかは提示順で決まる。

1. **要件サマリー** - 何を実装するか（1〜3 行。長文化させず、判断事項が冒頭 10 行以内に見えるようにする）
2. **判断事項** - 曖昧な要件に対する判断とその理由
3. **変更ファイル一覧** - 新規作成・修正するファイル
4. **実装順序** - 依存関係を考慮した実装ステップ
5. **ブランチ名** - 作成済みのブランチ名
...
---
PR のレビューと merge（人間の判断）待ちのため、Status を **Waiting** に移します。merge されると issue は自動 close されます。
EOF
)"
```

末尾の Waiting 行は Phase 9 の Status 遷移の「理由コメント」を兼ねる。§1.5 Step 2 で取得済みの options を見て次のとおり整える: `Waiting` があればこの行のまま／無く `Blocked` 等の代替 option を使う場合は文中の **Waiting** をその option 名に置き換える／代替も無く In Progress のまま据え置く場合（§1.5 で board 更新を skip した場合を含む）はこの行ごと削る。Waiting に動かすのにコメントを添えない状態・実際の Status と違う文言を投稿する状態のどちらも作らない。

## Phase 9: Waiting 遷移・クリーンアップ・完了報告
...
- **PR 作成と Issue コメント投稿が済んだら、board の Status を `Waiting` に移す** — 実装は終わり、残るのは人間のレビュー・merge（外部要因待ち）だから。§1.5 で board 更新を skip した場合（skip 条件に該当し ID 未取得）は、この Waiting 遷移も同様に skip し完了報告に一言添える。ID を取得済みなら、§1.5 Step 2 の ID を使い [`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §7 の mutation で `Waiting` の option に更新する。`Waiting` が無い board では `Blocked` 等「外部要因待ち」を意味する選択肢を使い、それも無ければ `In Progress` のまま動かさず完了報告に一言添える。
- **Status を Waiting に動かすときは、必ず理由コメントとセットにする** — 何を待っているか（PR のレビュー・merge／人間の判断事項／外部手続き）と再開条件が issue 上で辿れる状態にする。Phase 8 の PR コメント末尾の Waiting 行がこれを兼ねるので、通常は追加コメント不要。PR 以外の理由で人間待ちになった場合（判断材料を出して選択を委ねた等）は、その内容を書いたコメントを投稿してから Waiting に移す。コメント無しで Status だけ動かさない。
- PR が却下／作り直しになったら、再着手時に §1.5 をやり直す（Status も In Progress に戻る）。
