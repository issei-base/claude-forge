...
**実装計画の提示を含め、全フェーズを自動で完了させる。承認待ちで途中停止しない**（割り込みで修正指示が来たら都度反映する）。
...
### 範囲外の気づきを別 issue に逃がす

実装・テスト中に**本題の範囲外で**気づいた問題（バグ・設計の歪み・不足しているテスト・実態と合わなくなったドキュメント）は、今回の PR に混ぜず別 issue にする。混ぜると PR のスコープが崩れ、レビューと merge の判断が重くなる。

- **無人実行（headless）の場合**: [[create-issue]] で起票する（Status は触らないので `Backlog` に入る）。その場で相談できないので、起票しなければ気づきはそのまま失われる
- **対話の場合**: 完了報告に「気づいたこと」として並べ、起票するかはユーザに委ねる（会話で拾えるため先回りして起票しない）

**気づきが無ければ何もしない** — 件数を作ること自体に意味は無い。今回の変更で壊した / 壊しかけたものは「気づき」ではないので、Phase 6・7 で直しきる。
...
### 範囲外の気づき
- [起票した issue（無人実行時）/ 気づいた点の列挙（対話時）/ なし]
```
