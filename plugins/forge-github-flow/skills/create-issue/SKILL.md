...
3. **破壊的・不可逆・外向きの操作を含まない** — データ削除・マイグレーション実行・公開設定の変更・課金・インフラ apply は不適（コード変更として PR に閉じるものだけが対象）。**「merge は人間だから PR までなら安全」を前提にしない** — 2026-07-19 以降、CI が緑で安全ガード（`area: safety` / `area: data` ラベル・監修待ちゾーン・法務文書・migration・CI 設定）に当たらない PR は無人で merge される。この判定に「main に入って困るか」まで含める
