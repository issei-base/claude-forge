...
allowed-tools: Read, Write, Edit, Glob, Grep, Skill, Agent, Bash(git:*), Bash(gh pr view:*), Bash(gh pr edit:*), Bash(gh pr comment:*), Bash(gh pr checks:*), Bash(gh repo clone:*), Bash(gh run list:*), Bash(gh run view:*), Bash(gh issue view:*), Bash(gh issue comment:*), Bash(gh api:*), Bash(sleep:*), Bash(jq:*), Bash(ls:*), Bash(awk:*), Bash(tr:*), Bash(terraform fmt:*), Bash(terraform validate:*), Bash(terraform plan:*), Bash(prettier:*), Bash(npx prettier:*), Bash(eslint:*), Bash(npx eslint:*), Bash(ruff:*), Bash(golangci-lint:*)
---
...
- **PR に新しい振る舞いを足す修正（機能追加・挙動変更）は、テストを先に書いて落としてから実装する**（red→green。回帰固定テストの例外と申告規則は implement-issue Phase 6 と同一）。CI 赤の復旧・lint/format 対応・レビュー指摘の文言修正のみの場合は対象外
- 既存コードのスタイル・パターンに従う
...
push 完了後、Phase 6 が 3回FAILのまま打ち切りだった場合は警告コメント（Phase 6 サイクル末尾の heredoc）をここで投稿する。
...
- `now` が `GH_ERROR`（`gh` が非ゼロ終了）→ **空文字列を「全 PASS」に倒さない。** CI 状態を一度も取得できていないので、Step 8-5 で「CI 状態不明」として打ち切る。
- `now` が空 → 全 PASS。Phase 9 へ。
- `now` が非空 → FAIL あり。Step 8-3 へ。**`prev` と完全一致するなら無進捗**なので Step 8-5 で打ち切る。
- 4 回呼び直しても pending が残る → タイムアウト。Phase 10 の完了報告で「CI 未完了」を明記してユーザに引き継ぐ（worktree/PR は維持）。
...
- **無進捗**（`prev` と `now` の失敗 check 名が完全一致）→ サイクルが残っていても打ち切り、「無進捗のため打ち切り」を明記して Phase 9 へ（[`_shared/pr-conventions.md`](../_shared/pr-conventions.md) §5-4）。
- **CI 状態不明**（`now` が `GH_ERROR`）→ 全 PASS に倒さず打ち切り、`gh` が失敗した旨を明記して Phase 9 へ。
