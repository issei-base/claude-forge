...
| `Todo` ＋ `nightly` ラベル | **夜間ループの在庫。滞留を理由に戻さない**（残っているのが正常な状態で、毎晩上から消化される）。戻すのは issue 自体が不要になったときだけ |
...
器は「今週の消化見込み」であって、空の Todo に積む数ではない。**すでに Todo にあるものを必ず引く** — ここを引かないと Todo が毎週積み上がって第二の Backlog になる。

```
今週の新規枠 = 消化見込み − Todo の残数 − In Progress の残数
```

- ユーザが稼働・件数を明示していればそれに従う
- 明示が無ければ**消化見込みを次で見積もる**:
  - 夜間ループを運用していない repo → **5 件**（人間が手で着手する分）
  - 夜間ループがある repo（`nightly` ラベルが存在する）→ **1 晩の上限 × 7**（既定の `max_per_night: 2` なら 14 件）。無人で毎晩消化されるぶん器が大きくなる
- **枠が 0 以下なら新規は積まない**。Phase 5 では棚卸しの結果だけを提示し、「今週は在庫が足りている」と伝えて終わる（`Backlog` から機械的に補充しない）
...
**器**: 新規 <N> 件（消化見込み <C> − Todo 残 <T> − In Progress 残 <M>）
