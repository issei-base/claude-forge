...
- ターゲット列 = **「行 4 が埋まっていて行 5 が空」の最初の列**（最新レッスンの後）。誤って既存セルを上書きしない。
...
```python
# 列 1 (= 第1回) から見て、rows[3][n] が非空 かつ rows[4][n] が空 の最初の n
target_col = next(
    n for n in range(1, len(rows[3]))
    if rows[3][n].strip() and not rows[4][n].strip()
)
```

該当列が無い (全部埋まってる or 全部空) なら、ユーザーに状況を伝えて停止: 「最新レッスンの指導内容が未入力か、宿題が既に入っています」。
...
- ターゲット列の直前列 (= rows[4][target_col-1]) の宿題 = 直近の宿題と被らないように
- ターゲット列の直前列 (= rows[5][target_col-1]) = 「次回レッスン予定」= **今回作る宿題が結びつくべき方向性**
