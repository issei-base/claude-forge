...
| PR レビュー | Codex（ローカル CLI） | `ship` の push 前確認（§3.8）→ `codex-secondopinion`。GitHub 側 automatic review は既定オフ |
...
- `ship` — 人が確認しながら新規 PR を出す（自然言語「ship して/PR出して」で発火）。push 直前に Codex ローカルレビュー（`codex-secondopinion`）の要否を 1 回確認し（§3.8・`_shared/pr-conventions.md` §4）、PR 作成後は CI の収束ループ（最大 3 サイクル・`_shared` §3/§5 共有）まで自動で回す（**merge はしない**）。create-pr との差は「作成段階が対話的か非対話か」だけで、CI ループは共通。
...
- PR 作成は Claude Code に一本化する（`ship` / `create-pr` → feature branch → `gh pr create`）。PR レビューは **push 前のローカル `codex-secondopinion`** を標準にする（2026-07 方針転換: GitHub 側 automatic review + PR 後の応答ループはトークン消費とレビュー待ちが大きく廃止。例外的に GitHub 側 review が要る repo は Codex settings で手動有効化）。**PR 作成後に `@codex review` は投げない**。Codex は PR 作成をしない（`.agents/skills/ship` は review-only stub）。
