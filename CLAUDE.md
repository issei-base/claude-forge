...
- `ship` — 人が確認しながら新規 PR を出す（自然言語「ship して/PR出して」で発火）。PR 作成後は CI + Codex レビューの収束ループ（CI 最大 3 / Codex 最大 2 サイクル・`_shared/pr-conventions.md` §3/§4 共有）まで自動で回す（**merge はしない**）。create-pr との差は「作成段階が対話的か非対話か」だけで、収束ループは共通。
